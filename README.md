...
| `humanoid_retarget_defaults_hsi_hoi_true_scale.json` | OmniContact / OMOMO with true-size objects |
| `humanoid_retarget_defaults_nr.json` | NR FBX/BVH |
| `robot_configs/humanoid_retarget_defaults_adapt.json` | AdaPT SMPL-X+racket |

### True-Scale Scenes

By default the HSI/HOI pipeline scales the whole scene (human, objects, and
object trajectories) by the robot-to-human height ratio. Set
`solver.retarget_scene_mode` to `true_scale`, or pass `--scene-mode true_scale`,
to keep every object at its original size and source world pose instead. Only
the robot adapts, so object-object and object-ground placement is exactly the
source's, for any number of objects with a `prop_<name>.csv` in the sequence.

```bash
python scripts/humanoid_retarget_pipeline_hsi_hoi.py \
  --config robot_configs/humanoid_retarget_unitree_g1_example.json \
  --defaults humanoid_retarget_defaults_hsi_hoi_true_scale.json \
  --data sample_data/omomo \
  --seq-key sub1_plasticbox_015
```

The body is scaled about the ground point below the human root, so the robot
follows the human's path to the true-size objects. Contacts are detected
between the unscaled human and the scene, and each contact slot targets the
true object surface. The GRAIL defaults use this mode.

| Solver key | Meaning |
| --- | --- |
| `retarget_scene_mode` | `scaled` (default) or `true_scale`. |
| `true_scale_anchor` | `root_ground` (default), `contact_shift` to also shift the body horizontally toward contacted objects, or `origin`. |
| `interaction_mesh_cost` | Weight of the interaction-mesh term that keeps the local layout of the body and nearby object surface. |
| `interaction_mesh_radius` | Object points within this distance of the body join the interaction mesh. |
| `post_filter_refine_iters` | Optional QP iterations after the LQR filter to re-apply contacts and collisions. |

The retarget log prints `[SceneMetrics]` with contact-target error, near-contact
error, and robot-object penetration. True-scale results also store the object
poses the robot was solved against, and the viewer draws robot-side objects from
them at their original size. Carried objects stay at the human's height, so a
shorter robot must raise its arms, and contacts below the hands, such as a box
resting on the hips, cannot always be kept.

### Dexterous Hands

`robot_configs/humanoid_retarget_unitree_g1_rh56e2_example.json` retargets
onto a G1 with Inspire RH56E2 hands. `assets/g1_rh56e2/mujoco/g1_29dof_rh56e2_umr.xml`
is a flattened copy of the assembly made with `scripts/flatten_mjcf.py`.

```bash
python scripts/humanoid_retarget_pipeline_hsi_hoi.py \
  --config robot_configs/humanoid_retarget_unitree_g1_rh56e2_example.json \
  --defaults humanoid_retarget_defaults_hsi_hoi_embody.json
```

With `robot.hands.enabled`, each hand gets its own correspondence model next to
the body model: 512 slots per hand, learned in a wrist-aligned hand frame and
normalised per part (palm, thumb, index, middle, ring, pinky). A robot hand is
not a scaled human hand, so each part is mapped with its own proportions, and
the body slots that fell on the hands are replaced by the hand slots. The
pipeline builds, trains, merges, and caches both hand models automatically,
and writes a mapping viewer to
`output/<correspondence>/hand_correspondence_viewer/`:

```bash
python scripts/export_hand_correspondence_viewer.py --name <correspondence>
python -m http.server --directory output/<correspondence>/hand_correspondence_viewer 8000
```

The viewer shows the SMPL-X and robot hands side by side and colours slots by
correspondence, finger, or mismatch; clicking a slot highlights its partner.

- `retarget.zero_source_finger_pose: false` uses the source SMPL-X finger
  poses (`pose[75:165]`, flat-hand convention; set
  `retarget.source_hand_pose_convention: pca_mean` for poses relative to the
  mean hand). The default `true` keeps flat hands.
- `robot.joint_couplings: mjcf` enforces the MJCF `<equality><joint>`
  couplings exactly: the QP solves over independent joints only
  (`scripts/joint_couplings.py`). The default `off` leaves other robots
  unchanged.
- Each finger is its own surface segment; `robot.hands.segment_costs` sets
  `sample_slots`, `point_cost`, and `normal_cost` per finger and palm.
- The retarget log prints `[HandMetrics]` fingertip errors, in the world and in
  the robot hand frame, and the joint-coupling residual; both are saved in the
  result.
...
- MuJoCo treats a single mesh collision geom as its convex hull, which fills
  cavities such as a bowl or a microwave. The HSI/HOI pipeline splits any
  collision mesh whose hull is more than 5% larger than the mesh into
  [CoACD](https://github.com/SarahWeiii/CoACD) pieces before retargeting. It
  caches them under `output/object_collision_cache/` and writes the rewritten
  scene to `output/hsi_hoi_objects/<sequence>/`; the source folder is not
  modified. Objects that are already convex pieces are left as they are. The
  `hsi_hoi.object` keys `auto_convex_decomposition`, `coacd_concavity_tolerance`,
  and `coacd_*` control this.
